...
if getgenv().ForceMobileUI ~= nil then isMobile = getgenv().ForceMobileUI end
...
local currentTheme = holiday_themes[month] or { light=Color3.fromRGB(60,180,255), dark=Color3.fromRGB(20,25,40) }
...
if not is_mobile then
    UserInputService.InputBegan:Connect(function(key, is_processed)
        if is_processed then return end
        if key.KeyCode == Enum.KeyCode.P then
            ScreenGui.Enabled = not ScreenGui.Enabled
            if ScreenGui.Enabled then
                getgenv().notify("Success", "Showing performance statistics GUI.", 1)
            else
                getgenv().notify("Success", "Hiding performance statistics GUI.", 1)
            end
        end
    end)
end
