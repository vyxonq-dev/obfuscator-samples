...
local s, BulletHandler = pcall(require, RS.ModuleScripts.GunModules.BulletHandler);
if not s then
    return warn(executor .. " returned an error while trying to require RS.ModuleScripts.GunModules.BulletHandler:\n" .. BulletHandler);
end;
...
local getTarget = function(origin: Vector3)
    ...
    for _, player: Player in next, Players:GetPlayers() do
        ...
        if not char or char:FindFirstChild("INV") or (char:FindFirstChild("Humanoid") and char.Humanoid.Health <= 0) then continue; end;
...
local old; old = clonefunction(hookfunction(rawget(BulletHandler, "Fire"), newcclosure(function(args)
    if rawget(args, "Origin") and rawget(args, "Direction") then
        local c = getTarget(rawget(args, "Origin"));
        if c then
            local _args = args;
            _args.Direction = (c.Position - _args.Origin).Unit;
            return old(_args);
        end;
    end;
    return old(args);
