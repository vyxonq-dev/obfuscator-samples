...
-- NexusLib's Firebase "online" counter uses blocking HTTP every poll — leave false to avoid periodic hitches.
_G.NexusLib_EnablePresence = false
...
local states = {
    ...
    autoAllForOne=false, autoDioHeaven=false, autoMadara=false, autoAizenHogyoku=false,
    autoAreaBosses=false,
    mergeActive=false,
    farmNoTP=false, bossNoTP=false, farmActive=false, autoFarmIndex=false, indexTrackVariants=false, autoFarmSparking=false, autoFarmRifts=false, riftNoTP=false, autoRiftBossTeam=false,
    trialsActive=false, trialsNoTP=false, trialsNoLeave=false, autoTrialsBossTeam=false, trialsFast=false,
    serverHop=false, dragonBall=false, doorUnlock=false,
    fastForward=false, autoReExec=false, autoPVP=false, autoDailyQuest=false,
}
-- Full bright: apply from toggle / load only (avoids a permanent Heartbeat listener).
local _fbOrig={
...
local function applyFullBrightVisuals()
    if _fbActive then return end
    _fbActive=true
...
local function restoreFullBrightVisuals()
    if not _fbActive then return end
    _fbActive=false
    Lighting.Ambient=_fbOrig.Ambient
    Lighting.OutdoorAmbient=_fbOrig.OutdoorAmbient
    Lighting.Brightness=_fbOrig.Brightness
    Lighting.FogEnd=_fbOrig.FogEnd
    Lighting.FogStart=_fbOrig.FogStart
end
local walkSpeed      = 16
local hopDelay       = 10
local flyBV, flyBG
...
local leaveWave      = 5
trialDifficulty = trialDifficulty or "Normal"
...
local petNameList    = {}
local selectedDoors  = {}
selectedAreaBosses = selectedAreaBosses or {}
selectedRiftRanks = selectedRiftRanks or {}
selectedRiftPets = selectedRiftPets or {}
local farmTeam       = {}
local savedTeams     = {}
local farmTeamName   = ""
local isInBattle
local modePriority = {"AreaBosses","DailyQuest","RaidBosses","AutoFarm","Trials","AutoFarmIndex","AutoFarmSparking","AutoFarmRifts","PVP"}
local PRIORITY_LABELS = {
    ...
    AutoFarmRifts="Auto Farm Rifts",
    PVP="PVP",
}
local dailyQuestPriorityActive=false
AREA_BOSS_DEFAULTS = AREA_BOSS_DEFAULTS or {"Pain","Kars","Final Form Muzan","Awakened Mahito"}
RIFT_RANKS = RIFT_RANKS or {"F","D","C","B","A","S"}
RAID_BOSS_NAME_SET = RAID_BOSS_NAME_SET or {
    ["Boros"]=true,
    ["Fusion Zamasu (Corrupted)"]=true,
    ["Heian Era Sukuna"]=true,
    ["All For One"]=true,
    ["Dio Over Heaven"]=true,
    ["Madara Ten Tails"]=true,
    ["Aizen (Hogyoku)"]=true,
}
for _,name in ipairs(AREA_BOSS_DEFAULTS) do selectedAreaBosses[name]=true end
for _,rank in ipairs(RIFT_RANKS) do selectedRiftRanks[rank]=true end
...
local function encodeSettings()
    ...
    local abnames={}; for n,on in pairs(selectedAreaBosses) do if on then abnames[#abnames+1]=n end end
    data.selectedAreaBosses=table.concat(abnames,"|")
    local rranks={}; for r,on in pairs(selectedRiftRanks) do if on then rranks[#rranks+1]=r end end
    data.selectedRiftRanks=table.concat(rranks,"|")
    local rpnames={}; for n,on in pairs(selectedRiftPets) do if on then rpnames[#rpnames+1]=n end end
    data.selectedRiftPets=table.concat(rpnames,"|")
    local presetKeys={"Raid","AreaBosses","Farm","Rifts","RiftBosses","Trials","TrialsBosses","PVP"}
    ...
    data.trialDifficulty=tostring(trialDifficulty or "Normal")
    ...
    t[#t+1]='"trialDifficulty":"'..tostring(trialDifficulty or "Normal"):gsub('"',"'")..'"'
...
if LS.modePriority then
    local allowed={AreaBosses=true,DailyQuest=true,RaidBosses=true,Trials=true,AutoFarm=true,AutoFarmIndex=true,AutoFarmSparking=true,AutoFarmRifts=true,PVP=true}
    local parsed,seen={},{}
    ...
    for _,m in ipairs({"AreaBosses","DailyQuest","RaidBosses","AutoFarm","Trials","AutoFarmIndex","AutoFarmSparking","AutoFarmRifts","PVP"}) do
...
if type(LS.trialDifficulty)=="string" then
    local td=tostring(LS.trialDifficulty)
    if td=="Normal" or td=="Intermediate" or td=="Difficult" then
        trialDifficulty=td
    end
end
...
if LS.selectedAreaBosses then
    selectedAreaBosses={}
    for n in (LS.selectedAreaBosses.."|"):gmatch("([^|]+)|") do selectedAreaBosses[n]=true end
end
if LS.selectedRiftRanks then
    selectedRiftRanks={}
    for r in (LS.selectedRiftRanks.."|"):gmatch("([^|]+)|") do
        local rank=tostring(r or ""):upper()
        if rank~="" then selectedRiftRanks[rank]=true end
    end
end
if LS.selectedRiftPets then
    selectedRiftPets={}
    for n in (LS.selectedRiftPets.."|"):gmatch("([^|]+)|") do selectedRiftPets[n]=true end
end
do
    local presetKeys={"Raid","AreaBosses","Farm","Rifts","RiftBosses","Trials","TrialsBosses","PVP"}
    for _,pk in ipairs(presetKeys) do
...
if states.fullBright then task.defer(applyFullBrightVisuals) end
...
local prioModeOrder={"AreaBosses","DailyQuest","RaidBosses","AutoFarm","Trials","AutoFarmIndex","AutoFarmSparking","AutoFarmRifts","PVP"}
local prioInputs={}
...
FTab:Toggle("💡 Full Bright","Max lighting · removes fog",states.fullBright,function(on)
    ...
    if on then applyFullBrightVisuals() else restoreFullBrightVisuals() end
end)
...
FTab:Section("Daily Quest")
FTab:Toggle("📅 Auto Daily Quest","Fuse (farm fastest campaign pet), raid & rift dailies with correct teams, then claim. Reorder “Daily Quest” on the Priority tab.",states.autoDailyQuest,function(on)
    states.autoDailyQuest=on; saveSettings()
end)
...
FTab:Button("💬","Join Discord","Copies invite link to clipboard",C.accent,function()
    local invite="https://discord.gg/ErGUHtVK"
    local copied=false
    pcall(function()
...
local GetDailyData = game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("GetDailyData")
local ClaimDailyReward = game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("ClaimDailyReward")
local UpdateDailyInterface = game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("UpdateDailyInterface")
...
local presetDefs={
    ...
    {key="Rifts", icon="🌀", color=Color3.fromRGB(140,80,255), colorD=Color3.fromRGB(65,30,120)},
    {key="RiftBosses", icon="👺", color=Color3.fromRGB(255,105,75), colorD=Color3.fromRGB(120,45,30)},
    {key="Trials",icon="🏆", color=Color3.fromRGB(0,140,200),  colorD=Color3.fromRGB(0,60,100)},
    {key="TrialsBosses",icon="👹", color=Color3.fromRGB(230,90,60),  colorD=Color3.fromRGB(110,35,20)},
...
local function findTrialsModel()
    ...
    pcall(function()
        ...
        if lc then
            local kids=lc:GetChildren()
            -- User preference: always battle the first Metal Cooler in LocalCoolers.
            if kids[1] then
                result=kids[1]
                return
            end
            result=kids[1] or kids[#kids]
            return
        end
    end)
...
local function anyRaidBossEnabled()
    return states.autoBoros or states.autoZamasu or states.autoSukuna
        ...
        or states.autoAizenHogyoku
end

local function hasAliveAreaBoss()
    local c=getCampaign(); if not c then return false end
    for _,name in ipairs(AREA_BOSS_DEFAULTS) do
        if selectedAreaBosses[name] then
    ...
    local ok,res=pcall(function()
        ...
        for _,n in ipairs(ab:GetChildren()) do
            if selectedAreaBosses[n.Name] then
...
function getAreaBossFilterNames()
    local names,seen={},{}
    for _,n in ipairs(AREA_BOSS_DEFAULTS) do
        names[#names+1]=n
        seen[n]=true
    end
    pcall(function()
        ...
        for _,c in ipairs(ab:GetChildren()) do
            if (not RAID_BOSS_NAME_SET[c.Name]) and (not seen[c.Name]) then
                seen[c.Name]=true
...
function normalizeRiftRank(v)
    local s=tostring(v or ""):upper()
    local rank=s:match("([FDCBAS])")
    return rank
end
function getGateRank(gate)
    if not gate then return nil end
    local attrs={"Rank","RiftRank","gateRank","GateRank"}
    for _,a in ipairs(attrs) do
        local ok,val=pcall(function() return gate:GetAttribute(a) end)
        if ok and val~=nil then
            local rank=normalizeRiftRank(val)
            if rank then return rank end
        end
    end
    local marker=gate:FindFirstChild("RiftMarker")
    if marker then
        local lbl=marker:FindFirstChild("RiftRank")
        local txt=(lbl and (lbl.Text or lbl.ContentText)) or ""
        local rank=normalizeRiftRank(txt)
        if rank then return rank end
    end
    local rank=normalizeRiftRank(gate.Name)
    if rank then return rank end
    return "F"
end
function isRiftGateActive(gate)
    if not gate then return false end
    local ok,val=pcall(function() return gate:GetAttribute("Active") end)
    if ok and val~=nil then return val==true end
    return gate:FindFirstChild("RiftMarker")~=nil
end
function findJoinableRiftGate()
    local folder=workspace:FindFirstChild("RiftSpawns")
    if not folder then return nil,nil end
    for _,gate in ipairs(folder:GetChildren()) do
        if isRiftGateActive(gate) then
            local rank=getGateRank(gate)
            if selectedRiftRanks[rank] then
                return gate,rank
            end
        end
    end
...
recentRiftSpawns = recentRiftSpawns or {}
riftWasActive = riftWasActive or {}
function getGateRiftMapName(gate)
    if not gate then return "Unknown Rift" end
    local attrNames={"RiftName","riftName","MapName","RiftUnit"}
    ...
    local marker=gate:FindFirstChild("RiftMarker")
    if marker then
        local rn=marker:FindFirstChild("RiftName")
        local txt=tostring((rn and (rn.Text or rn.ContentText)) or "")
        if txt~="" then
            return txt
        end
    end
    return tostring(gate.Name or "Unknown Rift")
end
function pushRecentRiftSpawn(gate, rank)
    if not gate then return end
    local mapName=getGateRiftMapName(gate)
    local line=string.format("%s [%s]  %s", mapName, tostring(rank or "?"), os.date("%H:%M:%S"))
    table.insert(recentRiftSpawns,1,line)
    if #recentRiftSpawns>5 then
        table.remove(recentRiftSpawns,#recentRiftSpawns)
    end
end
local _riftPollLast=0
function pollRiftSpawnHistory()
    local now=os.clock()
    if now-_riftPollLast<2.5 then return end
    _riftPollLast=now
    local folder=workspace:FindFirstChild("RiftSpawns")
    ...
    for _,gate in ipairs(folder:GetChildren()) do
        alive[gate]=true
        local active=isRiftGateActive(gate)
        local prev=riftWasActive[gate] == true
        if active and not prev then
            pushRecentRiftSpawn(gate,getGateRank(gate))
        end
        riftWasActive[gate]=active
    end
    for gate,_ in pairs(riftWasActive) do
        if not alive[gate] then
            riftWasActive[gate]=nil
        end
    end
end
function preloadActiveRiftSpawns()
    local folder=workspace:FindFirstChild("RiftSpawns")
    if not folder then return end
    local seeded=0
    for _,gate in ipairs(folder:GetChildren()) do
        local active=isRiftGateActive(gate)
        riftWasActive[gate]=active
        if active and seeded<5 then
            pushRecentRiftSpawn(gate,getGateRank(gate))
            seeded=seeded+1
        end
    end
end
function getRiftPetsGroupedByMap()
    local groups={}
    pcall(function()
        local rs=game:GetService("ReplicatedStorage")
        local assets=rs:FindFirstChild("Assets")
        local maps=assets and assets:FindFirstChild("RiftMaps")
        if not maps then return end
        for _,map in ipairs(maps:GetChildren()) do
            local units=map:FindFirstChild("RiftUnits")
            if units then
                local names={}
                for _,u in ipairs(units:GetChildren()) do
                    names[#names+1]=u.Name
                end
                table.sort(names)
                groups[#groups+1]={map=map.Name,pets=names}
            end
        end
    end)
    table.sort(groups,function(a,b) return tostring(a.map)<tostring(b.map) end)
    return groups
end
function getActiveSelectedRiftModel()
    local campaign=getCampaign(); if not campaign then return nil,nil,nil end
    for _,model in ipairs(campaign:GetChildren()) do
        if model:GetAttribute("RiftUnit")~=nil and selectedRiftPets[model.Name] then
            local hum=model:FindFirstChildOfClass("Humanoid")
            local alive=(not hum) or hum.Health>0
            if alive then
                return model,model.Name,tostring(model:GetAttribute("RiftDifficulty") or "")
            end
        end
    end
    return nil,nil,nil
end
function isRiftBossModel(model)
    if not (model and model.Parent) then return false end
    local ok1,v1=pcall(function() return model:GetAttribute("IsBoss") end)
    if ok1 and v1==true then return true end
    local ok2,v2=pcall(function() return model:GetAttribute("BossUnit") end)
    if ok2 and v2==true then return true end
    local ok3,rd=pcall(function() return model:GetAttribute("RiftDifficulty") end)
    if ok3 and rd~=nil then
        local r=tostring(rd):upper()
        if r=="A" or r=="S" then return true end
    end
    local gui=model:FindFirstChild("Gui")
    if gui and gui:FindFirstChild("BossLabel") then return true end
    if model:FindFirstChild("BossLabel",true)~=nil then return true end
    if gui then
        for _,d in ipairs(gui:GetDescendants()) do
            if d:IsA("TextLabel") then
                local t=tostring(d.ContentText or d.Text or ""):upper()
                if t:find("BOSS",1,true) then return true end
            end
        end
    end
    local n=tostring(model.Name or ""):lower()
    if n:find("boss",1,true) or n:find("final form",1,true) then return true end
    return false
end
...
local RAID_BOSS_BY_TOGGLE={
    ...
    autoAizenHogyoku="Aizen (Hogyoku)",
}
...
local function countMissingIndexTargets()
    local now=os.clock()
    if now-_missingIndexAt<2.5 and _missingIndexCount>=0 then
...
local _sparkScanLast=0
local _sparkScanVal=false
local function hasSafariSparkingTargetCached()
    local now=os.clock()
    if now-_sparkScanLast<3 then return _sparkScanVal end
    _sparkScanVal=hasSafariSparkingTarget()
    _sparkScanLast=now
    return _sparkScanVal
end

local function isModeBlocking(mode)
    if mode=="DailyQuest" then return states.autoDailyQuest and dailyQuestPriorityActive end
    ...
    if mode=="AutoFarmRifts" then return states.autoFarmRifts and getActiveSelectedRiftModel()~=nil end
...
-- Chunk-level: error loops below assign this; Status UI reads it (must not be trapped in an inner block).
...
local riftCard=Instance.new("Frame",PStatus)
riftCard.Size=UDim2.new(1,-10,0,110); riftCard.BackgroundColor3=C.card; riftCard.BorderSizePixel=0; riftCard.LayoutOrder=210
Instance.new("UICorner",riftCard).CornerRadius=UDim.new(0,10)
Instance.new("UIStroke",riftCard).Color=C.border
local riftTitle=Instance.new("TextLabel",riftCard)
riftTitle.Text="Last 5 Rift Spawns"; riftTitle.Size=UDim2.new(1,-16,0,16); riftTitle.Position=UDim2.new(0,10,0,6)
riftTitle.BackgroundTransparency=1; riftTitle.TextColor3=C.sub; riftTitle.Font=Enum.Font.GothamBold; riftTitle.TextSize=9; riftTitle.TextXAlignment=Enum.TextXAlignment.Left
local riftLbl=Instance.new("TextLabel",riftCard)
riftLbl.Size=UDim2.new(1,-16,1,-26); riftLbl.Position=UDim2.new(0,10,0,22)
riftLbl.BackgroundTransparency=1; riftLbl.TextColor3=C.text; riftLbl.Font=Enum.Font.Code; riftLbl.TextSize=9
riftLbl.TextXAlignment=Enum.TextXAlignment.Left; riftLbl.TextYAlignment=Enum.TextYAlignment.Top
riftLbl.TextWrapped=true; riftLbl.Text="(waiting for rift spawns...)"
preloadActiveRiftSpawns()

local startedAt=os.clock()
local lastNow=""
local recent={}
local lastStatusSnap=""
...
task.spawn(function()
    while true do
        ...
        local sparkState=(not states.autoFarmSparking and "OFF") or (hasSafariSparkingTargetCached() and "FOUND") or "WAITING"
        ...
        pollRiftSpawnHistory()
        if #recentRiftSpawns>0 then
            riftLbl.Text=table.concat(recentRiftSpawns,"\n")
        else
            riftLbl.Text="(waiting for rift spawns...)"
        end
        if current~=lastStatusSnap then
            lastStatusSnap=current
            pushRecent(current)
        end
        task.wait(2.1)
    end
...
ABTab:Section("Area Bosses")
ABTab:Toggle("👑 Auto Area Bosses","Auto battle AreaBosses when they spawn",states.autoAreaBosses,function(on)
    states.autoAreaBosses=on; saveSettings()
end)
ABTab:Section("Area Boss Filter")
for _,bossName in ipairs(getAreaBossFilterNames()) do
    local defaultOn=selectedAreaBosses[bossName]
    if defaultOn==nil then
        defaultOn=true
        selectedAreaBosses[bossName]=true
    end
    ABTab:Toggle("☑ "..bossName,"Fight this area boss when active",defaultOn,function(on)
        selectedAreaBosses[bossName]=on
        saveSettings()
    end)
end
...
local bossConfig={
    ...
    {"🧪","Aizen (Hogyoku)","Aizen (Hogyoku)","autoAizenHogyoku",16,"Aizen (Hogyoku)"},
}
...
RTab:Section("Rifts")
RTab:Toggle("⚔ Auto Farm Rift Pets","Auto battle selected rift pets in Campaign",states.autoFarmRifts,function(on)
    states.autoFarmRifts=on; saveSettings()
end)
RTab:Toggle("👺 Auto Rift Boss Team","Detect rift bosses and switch to RiftBosses preset",states.autoRiftBossTeam,function(on)
    states.autoRiftBossTeam=on; saveSettings()
end)
RTab:Toggle("🚫 No TP (Rifts)","Farm selected rift pets without teleporting",states.riftNoTP,function(on)
    states.riftNoTP=on; saveSettings()
end)
RTab:Section("Rift Pet Targets")
for _,group in ipairs(getRiftPetsGroupedByMap()) do
    RTab:Section("📍 "..group.map)
    for _,petName in ipairs(group.pets) do
        local on=selectedRiftPets[petName]
        if on==nil then
            on=false
            selectedRiftPets[petName]=false
        end
        RTab:Toggle("☑ "..petName,"Farm this rift pet when active",on,function(v)
            selectedRiftPets[petName]=v
            saveSettings()
        end)
    end
end
RTab:Section("Rift Map Tools")
RTab:Button("📜","Print Rift Map Pets","Reads ReplicatedStorage.Assets.RiftMaps.<Rift>.RiftUnits names",C.accent,function()
    local ok,err=pcall(function()
        local rs=game:GetService("ReplicatedStorage")
        local assets=rs:FindFirstChild("Assets")
        local maps=assets and assets:FindFirstChild("RiftMaps")
        if not maps then
            SLbl.Text="⚠ RiftMaps folder not found"
            return
        end
        local out={}
        for _,map in ipairs(maps:GetChildren()) do
            ...
            if units then
                ...
                table.sort(names)
                out[#out+1]=map.Name..": "..(#names>0 and table.concat(names,", ") or "(no units)")
            end
        end
        table.sort(out)
        print("===== Rift Map Pets =====")
        for _,line in ipairs(out) do print(line) end
        print("===== End Rift Map Pets =====")
        SLbl.Text="Printed "..tostring(#out).." rift map unit lists"
    end)
    if not ok then
        SLbl.Text="⚠ Rift map scan failed: "..tostring(err):sub(1,60)
    end
end)
...
TTab:Section("Difficulty")
local trialDiffLabel=TTab:Label("Selected: "..tostring(trialDifficulty or "Normal"))
local function setTrialDifficulty(v)
    if v~="Normal" and v~="Intermediate" and v~="Difficult" then return end
    trialDifficulty=v
    trialDiffLabel.Set("Selected: "..v)
    saveSettings()
end
TTab:Button("🟢","Normal","Use Normal trial difficulty",C.green,function() setTrialDifficulty("Normal") end)
TTab:Button("🟡","Intermediate","Use Intermediate trial difficulty",C.gold,function() setTrialDifficulty("Intermediate") end)
TTab:Button("🔴","Difficult","Use Difficult trial difficulty",C.red,function() setTrialDifficulty("Difficult") end)
TTab:Toggle("👹 Auto Trials Boss Team","Detect boss phase and switch to TrialsBosses preset",states.autoTrialsBossTeam,function(on)
    states.autoTrialsBossTeam=on; saveSettings()
end)
TTab:Toggle("⚡ Fast Trials","Classic runner · uses Difficulty + WaveLabel · same waits as slim script",states.trialsFast,function(on)
    states.trialsFast=on; saveSettings()
end)
...
--  Wrapped in its own function so Luau's 200 local register limit is not shared with the whole script.
-- ============================================================
...
-- Rebuilding the merge pet scroll is expensive (inventory scan + dozens of UI rows).
-- Debounce so rapid auto-merge does not freeze the client every 0.4s.
local _mergePetListDebounceTok=0
local function scheduleMergePetListRefresh()
    _mergePetListDebounceTok=_mergePetListDebounceTok+1
    local tok=_mergePetListDebounceTok
    task.delay(2.2,function()
        if tok~=_mergePetListDebounceTok then return end
        if RUN_GEN~=_G.AniPixels_RunGen then return end
        pcall(function() buildPetList(SearchBox.Text) end)
    end)
end

task.spawn(function()
    local activeRiftTeamKey="Rifts"
    while true do
        if RUN_GEN~=_G.AniPixels_RunGen then break end
...
task.spawn(function()
    while true do
        ...
        if states.mergeActive and mergeMaxTier>=4 then
            ...
            task.wait(clicked and 0.25 or 0.15)
        else
            task.wait(0.2)
        end
    end
...
-- Used by door-unlock loop below this IIFE; must be chunk locals, not trapped inside.
local DOOR_CONFIG={}
local DoorStatusLbl
local checkOwned
...
local _lastGodHb=0
-- Single Heartbeat: skip entirely when nothing needs per-frame work (default: all features off).
RunService.Heartbeat:Connect(function()
    if RUN_GEN~=_G.AniPixels_RunGen then return end
    if not states.noclip and not states.godMode and not states.fly then
        if flyBV then flyBV:Destroy(); flyBV=nil end
        if flyBG then flyBG:Destroy(); flyBG=nil end
        return
    end
...
-- Daily quest: fuse farm, raids/rifts (correct presets), optional trials start, claim
local DAILY_RAID_PATHS={"Boros","Fusion Zamasu (Corrupted)","Heian Era Sukuna","All For One","Dio Over Heaven","Madara Ten Tails","Aizen (Hogyoku)"}
local function dailyChallengeTitleNeeds(ch,words)
    if type(ch)~="table" then return false end
    for title,v in pairs(ch) do
        local h=tonumber(v and v.Has) or 0
        local n=tonumber(v and v.Needed) or 1
        if h<n then
            local tl=string.lower(tostring(title))
            local ok=true
            for wi=1,#words do
                if not tl:find(words[wi],1,true) then ok=false break end
            end
...
local function pickBestBaseForDailyFuse(needGiant,needTitan)
    if #petNameList==0 then refreshPetCampaignMap() end
    local bases={}
    for _,b in ipairs(petNameList) do bases[b]=true end
    if next(bases)==nil then
        local dInv=getDInv()
        if dInv then
            for _,iv in ipairs(dInv:GetChildren()) do
                if iv:IsA("IntValue") and iv.Value>0 then
                    local base=toMergeBaseName(iv.Name)
                    if base~="" then bases[base]=true end
                end
            end
        end
    end
    ...
    for base in pairs(bases) do
        local b=getInvCountByName(base)
        local g=getInvCountByName("Giant "..base)
        local m=getInvCountByName("Mega "..base)
        local sc=b+g*3+m*9
        if needGiant or needTitan then
            if m>=4 then sc=sc+5000 elseif g>=4 then sc=sc+3000 elseif b>=4 then sc=sc+1000 end
        end
        if sc>score then score,best=sc,base end
    end
    return best
end
local function selectDailyMergePetName(base,needGiant,needTitan)
    if not base or base=="" then return nil end
    if needTitan then
        if getInvCountByName("Mega "..base)>=4 then return "Mega "..base end
        if getInvCountByName("Giant "..base)>=4 then return "Giant "..base end
    end
    if needGiant or needTitan then
        if getInvCountByName(base)>=4 then return base end
    end
    return nil
end
local _dailyDataCache=nil
local _dailyDataCacheAt=0
local DAILY_DATA_CACHE_SEC=8
pcall(function()
    UpdateDailyInterface.OnClientEvent:Connect(function()
        _G.AniPixels_DailyQuestRefresh = true
    end)
end)
task.spawn(function()
    while true do
        if RUN_GEN~=_G.AniPixels_RunGen then break end
        if not states.autoDailyQuest then
            dailyQuestPriorityActive=false
            _dailyDataCache=nil
            task.wait(2)
        else
            local loopOk,loopErr=pcall(function()
                local nowFetch=os.clock()
                local forceFresh=_G.AniPixels_DailyQuestRefresh
                if forceFresh then _G.AniPixels_DailyQuestRefresh=false end
                local data
                if not forceFresh and type(_dailyDataCache)=="table" and (nowFetch-_dailyDataCacheAt)<DAILY_DATA_CACHE_SEC then
                    data=_dailyDataCache
                else
                    data=GetDailyData:InvokeServer()
                    if type(data)=="table" then
                        _dailyDataCache=data
                        _dailyDataCacheAt=nowFetch
                    end
                end
                if type(data)~="table" then
                    dailyQuestPriorityActive=false
                ...
                local ch=data.challenges
                if type(ch)~="table" or next(ch)==nil then
                    dailyQuestPriorityActive=false
                    task.wait(18)
                    return
                end
                ...
                for _,v in pairs(ch) do
                    local h=tonumber(v and v.Has) or 0
                    local n=tonumber(v and v.Needed) or 1
                    if h<n then allDone=false break end
                end
                dailyQuestPriorityActive=not allDone
                if allDone then
                    local okC,success,msg=pcall(function()
                        return ClaimDailyReward:InvokeServer()
                    end)
                    if okC and success==true then
                        pcall(function() SLbl.Text="📅 Daily reward claimed! "..tostring(msg or "") end)
                        dailyQuestPriorityActive=false
                        _dailyDataCache=nil
                        task.wait(8)
                    elseif okC then
                        pcall(function() SLbl.Text="📅 Daily claim: "..tostring(msg or "failed") end)
                        task.wait(30)
                    else
                        task.wait(20)
                    end
                ...
                if not canRunMode("DailyQuest") then
                    task.wait(1)
                    return
                end
                if isInBattle() then
                ...
                local needGiant=dailyChallengeTitleNeeds(ch,{"fuse","giant"})
                local needTitan=dailyChallengeTitleNeeds(ch,{"fuse","titan"})
                local needRaid=dailyChallengeTitleNeeds(ch,{"raid"})
                local needRift=false
                for title,v in pairs(ch) do
                    local h=tonumber(v and v.Has) or 0
                    local n=tonumber(v and v.Needed) or 1
                    if h<n then
                        local tl=string.lower(tostring(title))
                        if tl:find("rift",1,true) then needRift=true end
                    end
                end
                if needGiant or needTitan then
                    local base=pickBestBaseForDailyFuse(needGiant,needTitan)
                    if base then
                        local mergeName=selectDailyMergePetName(base,needGiant,needTitan)
                        if mergeName then
                            pcall(function() SLbl.Text="📅 Daily fuse: "..mergeName end)
                            local squad=makeMergePayload(mergeName)
                            pcall(function() RemoteMerge:InvokeServer(squad) end)
                            task.wait(0.45)
                            _G.AniPixels_DailyQuestRefresh=true
                            return
                        end
                        pcall(function() SLbl.Text="📅 Daily farm (fuse): "..base end)
                        applyPresetTeam("Farm")
                        task.wait(0.2)
                        local model=findCampaignModelForPet(base)
                        if model and isPetSpawnedInModel(model,base) then
                            if not states.farmNoTP then
                                pcall(function()
                                    local root=getRoot()
                                    if root then
                            ...
                            if isPetSpawnedInModel(model,base) then
                                pcall(function() RequestBattle:FireServer(model) end)
                                task.wait(1)
                                return
                            end
                ...
                if needRaid then
                    for _,path in ipairs(DAILY_RAID_PATHS) do
                        if isBossPresent(path) and not isBossDead(path) then
                            applyPresetTeam("Raid")
                            task.wait(0.2)
                            local bossModel=nil
                            pcall(function()
                                local c=getCampaign()
                                bossModel=c and c:FindFirstChild(path)
                            end)
                            if bossModel then
                                ...
                                pcall(function() SLbl.Text="📅 Daily raid: "..path end)
                ...
                if needRift then
                    local model,petName=getActiveSelectedRiftModel()
                    if model then
                        local desired="Rifts"
                        if states.autoRiftBossTeam and isRiftBossModel(model) then
                            desired="RiftBosses"
                        end
                        applyPresetTeam(desired)
                        task.wait(0.2)
                        if not states.riftNoTP then
                            pcall(function()
                                local root=getRoot()
                                if root then
                                    local pos=model:GetModelCFrame().Position
                        ...
                        pcall(function() SLbl.Text="📅 Daily rift: "..tostring(petName) end)
                        pcall(function() RequestBattle:FireServer(model) end)
                        task.wait(autoFightDelay)
                        return
                    end
                ...
                local w=12
                if _G.AniPixels_DailyQuestRefresh then _G.AniPixels_DailyQuestRefresh=false w=4 end
                task.wait(w)
            end)
            if not loopOk then
                dailyQuestPriorityActive=false
                statusLastError="DailyQuest: "..tostring(loopErr):sub(1,80)
                pcall(function() SLbl.Text="⚠ Daily quest check: "..tostring(loopErr):sub(1,50) end)
                task.wait(20)
            end
        end
    end
...
bossLoop("autoAizenHogyoku","Aizen (Hogyoku)")

task.spawn(function()
    while true do
        if RUN_GEN~=_G.AniPixels_RunGen then break end
        local loopOk,loopErr=pcall(function()
            if not states.autoFarmRifts then task.wait(0.35); return end
            if not canRunMode("AutoFarmRifts") then task.wait(0.6); return end
            if isInBattle() then task.wait(0.9); return end
            local model,petName,rank=getActiveSelectedRiftModel()
            if not model then
                SLbl.Text="🌀 No selected active rift pets"
                task.wait(1)
                return
            end
            local desiredTeam="Rifts"
            if states.autoRiftBossTeam and isRiftBossModel(model) then
                desiredTeam="RiftBosses"
            end
            if desiredTeam~=activeRiftTeamKey then
                applyPresetTeam(desiredTeam); task.wait(0.2)
                activeRiftTeamKey=desiredTeam
            else
                applyPresetTeam(desiredTeam); task.wait(0.2)
            end
            if not states.riftNoTP then
            ...
            pcall(function() RequestBattle:FireServer(model) end)
            SLbl.Text="🌀 Rift Farm ("..activeRiftTeamKey.."): "..petName..(rank~="" and (" ["..rank.."]") or "")
            task.wait(autoFightDelay)
        end)
        if not loopOk then
            statusLastError="RiftFarmLoop: "..tostring(loopErr):sub(1,80)
            pcall(function() SLbl.Text="⚠ Rift farm error (recovering): "..tostring(loopErr):sub(1,50) end)
            task.wait(2)
        end
    end
end)
...
task.spawn(function()
    ...
    while true do
        ...
        local loopOk,loopErr=pcall(function()
            ...
            -- Classic / slim-script path: same Start payload as full (incl. Difficulty), cached WaveLabel, fixed waits, no boss/objective/GUI-restart extras.
            if states.trialsFast then
                TSLbl.Text="▶ Starting trial session..."
                local startOk,startErr=pcall(function()
                    TrialsMode:FireServer({Start=true,Difficulty=tostring(trialDifficulty or "Normal")})
                end)
                ...
                local function getWaveFast()
                    local wl=waveLabel
                    if not (wl and wl.Parent) then return 0 end
                    local n=tonumber((tostring(wl.ContentText or wl.Text or "")):match("%d+"))
                    return n or 0
                end
                local currentWave=getWaveFast(); local sessionAlive=true
                local lastTp=0
                while states.trialsActive and sessionAlive do
                    if RUN_GEN~=_G.AniPixels_RunGen then break end
                    if not canRunMode("Trials") then
                        TSLbl.Text="⏸ Leaving Trials — higher-priority mode"
            ...
            if not trialsPreStarted then
                ...
                trialsPreStarted=false
                TSLbl.Text="▶ Continuing restarted trial session..."
            end
            local battleModel=nil
            for _=1,6 do battleModel=findTrialsModel(); if battleModel then break end; task.wait(0.5) end
            if not battleModel then TSLbl.Text="⚠ No cooler model found after 3s"; task.wait(2); return end
            local coolerIndex=1; pcall(function() coolerIndex=battleModel:GetAttribute("CoolerIndex") or 1 end)
            applyPresetTeam("Trials"); task.wait(0.2)
            local activeTrialTeamKey="Trials"
            if not states.trialsNoTP then
                pcall(function()
                    local root=getRoot(); if not root then return end
                    local hrp=battleModel:FindFirstChild("HumanoidRootPart")
            ...
            local function getTrialsLabels()
                local sg=playerGui:FindFirstChild("ScreenGui")
                local tf=sg and sg:FindFirstChild("Trials")
                if not tf then return nil,nil end
                return tf:FindFirstChild("WaveLabel"), tf:FindFirstChild("ObjectiveLabel")
            end
            local function getWave()
                local wl=getTrialsLabels()
                if not (wl and wl.Parent) then return 0 end
                local txt=tostring(wl.ContentText or wl.Text or "")
                local n=tonumber(txt:match("%d+"))
                return n or 0
            end
            local function hasWaveGui()
                local wl=getTrialsLabels()
                return wl and wl.Parent and true or false
            end
            local function isObjectiveComplete()
                local _,ol=getTrialsLabels()
                if not (ol and ol.Parent) then return true end
                local txt=tostring(ol.ContentText or ol.Text or "")
                local lower=txt:lower()
                if lower:find("complete",1,true) or lower:find("completed",1,true) or lower:find("done",1,true) then
                    return true
                end
                local a,b=txt:match("%((%d+)%s*/%s*(%d+)%)")
                local done=tonumber(a)
                local need=tonumber(b)
                if not done or not need or need<=0 then return false end
                return done>=need
            end
            local function isTrialsBossNow(waveNow)
                local bm=findTrialsModel()
                if bm and bm.Parent then
                    if tostring(bm.Name)=="Cooler Final Form" then return true end
                    local ok,isBoss=pcall(function() return bm:GetAttribute("IsBoss") end)
                    if ok and isBoss==true then return true end
                    local bossLabelFound=false
                    pcall(function()
                        local gui=bm:FindFirstChild("Gui")
                        if gui and gui:FindFirstChild("BossLabel") then
                            bossLabelFound=true
                            return
                        end
                        if bm:FindFirstChild("BossLabel",true)~=nil then bossLabelFound=true end
                    end)
                    if bossLabelFound then return true end
                end
                local _,ol=getTrialsLabels()
                if ol and ol.Parent then
                    local ot=tostring(ol.ContentText or ol.Text or ""):lower()
                    if ot:find("final form",1,true) then
                        return true
                    end
                end
                return (tonumber(waveNow) or 0)>=100
            end
            local function isTrialsBossModel(model)
                if not (model and model.Parent) then return false end
                if tostring(model.Name)=="Cooler Final Form" then return true end
                local ok,isBoss=pcall(function() return model:GetAttribute("IsBoss") end)
                if ok and isBoss==true then return true end
                local hasBossLabel=false
                pcall(function()
                    local gui=model:FindFirstChild("Gui")
                    if gui and gui:FindFirstChild("BossLabel") then
                        hasBossLabel=true
                    ...
                    if model:FindFirstChild("BossLabel",true)~=nil then hasBossLabel=true end
                end)
                return hasBossLabel
            end
            local currentWave=getWave(); local sessionAlive=true
            local lastTp=0
            local missingWaveGuiTicks=0
            while states.trialsActive and sessionAlive do
                if RUN_GEN~=_G.AniPixels_RunGen then break end
                if not canRunMode("Trials") then
                    TSLbl.Text="⏸ Leaving Trials — higher-priority mode"
                ...
                if not hasWaveGui() then
                    missingWaveGuiTicks=missingWaveGuiTicks+1
                    TSLbl.Text="⚠ Wave GUI missing ("..missingWaveGuiTicks.."/6) — waiting..."
                    if missingWaveGuiTicks>=6 then
                        TSLbl.Text="🔁 Wave GUI not found — restarting Trials..."
                        pcall(function() TrialsMode:FireServer({Exit=true}) end)
                        task.wait(0.8)
                        pcall(function()
                            TrialsMode:FireServer({Start=true,Difficulty=tostring(trialDifficulty or "Normal")})
                        end)
                        trialsPreStarted=true
                        sessionAlive=false
                        break
                    end
                    task.wait(0.5)
                    ...
                    missingWaveGuiTicks=0
                end
                currentWave=getWave()
                if states.autoTrialsBossTeam then
                    local desiredTeam=isTrialsBossNow(currentWave) and "TrialsBosses" or "Trials"
                    if desiredTeam~=activeTrialTeamKey then
                        applyPresetTeam(desiredTeam); task.wait(0.2)
                        activeTrialTeamKey=desiredTeam
                    end
                end
                TSLbl.Text="⚔ Wave "..currentWave.."/"..(states.trialsNoLeave and "∞" or tostring(leaveWave))
                if not states.trialsNoLeave and currentWave>=leaveWave then
                    if leaveWave>=100 and not isObjectiveComplete() then
                        TSLbl.Text="🎯 Wave "..currentWave.." reached, farming until objective complete..."
                    else
                        if leaveWave>=100 then
                            TSLbl.Text="✅ Objective complete — restarting in 5s..."
                            task.wait(5)
                            TSLbl.Text="🔁 Re-firing Trials start..."
                            pcall(function() TrialsMode:FireServer({Exit=true}) end)
                            task.wait(0.8)
                            pcall(function()
                                TrialsMode:FireServer({Start=true,Difficulty=tostring(trialDifficulty or "Normal")})
                ...
                local freshModel=findTrialsModel()
                if freshModel then
                    battleModel=freshModel
                    pcall(function() coolerIndex=battleModel:GetAttribute("CoolerIndex") or coolerIndex end)
                end
                if states.autoTrialsBossTeam then
                    local desiredTeam=(isTrialsBossModel(battleModel) or isTrialsBossNow(currentWave)) and "TrialsBosses" or "Trials"
                    if desiredTeam~=activeTrialTeamKey then
                        applyPresetTeam(desiredTeam)
                        task.wait(0.15)
                        activeTrialTeamKey=desiredTeam
                    end
                end
