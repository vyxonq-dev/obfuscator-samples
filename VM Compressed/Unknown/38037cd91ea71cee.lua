...
local s, Projectile = pcall(require, RS.ModuleScripts.GunModules.GunFramework.Projectile);
if not s then
    return warn(executor .. " returned an error while trying to require RS.ModuleScripts.GunModules.GunFramework.Projectile:\n" .. Projectile);
end;
...
local getTarget = function(origin: Vector3)
    ...
    for _, player: Player in next, Players:GetPlayers() do
        ...
        local tPart: BasePart = char:FindFirstChild("Crit") or char:FindFirstChild("HitBox") or char.PrimaryPart;
...
local old; old = clonefunction(hookfunction(rawget(Projectile, "Cast"), newcclosure(function(_, __, origin, ___, ...)
    local c = getTarget(origin);
    if c then
        return old(_, __, origin, c.Position, ...);
    end;
    return old(_, __, origin, ___, ...);
end)));
