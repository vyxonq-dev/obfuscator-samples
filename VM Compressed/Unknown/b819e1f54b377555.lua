...
local s2, rec = pcall(require, RS.Modules.Weapons.Recoil);
if not s2 then
    return warn(executor .. " returned an error while trying to require RS.Modules.Weapons.Recoil:\n" .. rec);
end;
...
local getTarget = function(origin: Vector3)
    ...
    local collection = char.Parent.Name == "Survivors" and Chars:QueryDescendants("Folder#Bots>Model, Folder#Infections>Model") or Chars.Survivors:GetChildren();

    for _, character: Model in next, collection do
        if character:FindFirstChildOfClass("ForceField") or (character:FindFirstChild("Humanoid") and character.Humanoid.Health <= 0) then continue; end;

        local tPart: BasePart = character:FindFirstChild("Head") or character.PrimaryPart or character:FindFirstChild("HumanoidRootPart");
        ...
        if not v then
            v, nTPart = isVisible(character.PrimaryPart or character:FindFirstChild("HumanoidRootPart"), origin);
...
rawset(rec, "Recoil", function() return; end);
