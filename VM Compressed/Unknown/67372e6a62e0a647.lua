-- Shitty X Unified Macro (UI + Recorder + Player) - Fixed
...
local macroDropdown = playtab:Dropdown({
	Label = "Select Macro",
	Items = getMacros(),
	Callback = function(v)
		env.selectedMacro = v
...
playtab:Button({
	Text = "Refresh Macros",
	Callback = function()
		macroDropdown:SetItems(getMacros())
		updatelog("Macro list refreshed")
	end
})
...
playtab:Button({
	...
	Callback = function()
		...
		updatelog("Playing macro: " .. env.selectedMacro)
