...
local Window = Rayfield:CreateWindow({
   ...
   KeySettings = {
      Title = "XYLO HUB",
      Subtitle = "ARSENAL",
      Note = "Key in https://discord.gg/4cB8Djgauv", -- Use this to tell the user how to get a key
...
local TEAM_CHECK = true  -- Toggle für Team-Check
...
local function getClosestTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer 
            ...
            and (not TEAM_CHECK or player.Team ~= LocalPlayer.Team) -- Team-Check hier
        then
...
local TEAM_CHECK = true -- Team-Check
local TELEPORT_HEIGHT = 3 -- Höhe über dem Kopf

local autofarm = false
local connection

-- Funktion, um den nächsten Gegner zu finden
local function getNextTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer 
            ...
            and player.Character:FindFirstChild("Humanoid") 
            and player.Character.Humanoid.Health > 0
            and (not TEAM_CHECK or player.Team ~= LocalPlayer.Team)
        then
            local part = player.Character:FindFirstChild("Head") or player.Character:FindFirstChild("HumanoidRootPart")
            if part then
...
local function startAutofarm()
    connection = RunService.RenderStepped:Connect(function()
        if not autofarm or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
        
        local target = getNextTarget()
        if target then
            -- Teleport über den Kopf
            local newPos = target.Position + Vector3.new(0, TELEPORT_HEIGHT, 0)
            -- Blick direkt auf den Kopf
            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(newPos, target.Position)
        end
    end)
end
...
local Toggle = AutoTab:CreateToggle({
    Name = "Autofarm (doesnt work in ffa, you have to shoot yourself)",
    CurrentValue = false,
    Flag = "AF",
    Callback = function(Value)
        autofarm = Value
...
local VisualTab = Window:CreateTab("👁️ VISUAL", nil) -- Title, Image
