...
local s, gcm = pcall(require, RS.ModuleScript.GameCommonMethod);
if not s then
    return warn(executor .. " returned an error while trying to require RS.ModuleScript.GameCommonMethod:\n" .. gcm);
end;
...
local getTarget = function(origin: Vector3)
    ...
    for _, player: Player in next, Players:GetPlayers() do
        ...
        if not char or (char:FindFirstChild("Humanoid") and char.Humanoid.Health <= 0) or (char:FindFirstChild("Head") and char.Head:FindFirstChild("TeamMate")) then continue; end;
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/sneekygoober/sneeky-s-fov-lib/refs/heads/main/main.luau"))()(getgenv().fov or 300, getTarget);

local old; old = clonefunction(hookfunction(rawget(gcm, "Raycast"), newcclosure(function(origin, _, ...)
    local c = getTarget(origin);
    if c then
        local dir = c.Position - origin;
        return old(origin, dir.Unit * dir.Magnitude, ...);
    end;
    return old(origin, _, ...);
end)));
