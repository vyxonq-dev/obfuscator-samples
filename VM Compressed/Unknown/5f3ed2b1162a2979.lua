-- this script is not mine only modified by me (l0st veil) and then modified by me lol (FlamesW)
-- credits to the @HeardKometa on v3rm https://v3rm.net/threads/release-free-private-servers-for-any-game.23903/
-- getgenv().SendInvites = false
if not game:IsLoaded() then game.Loaded:Wait() end
if not isfolder("PrivateServerGenerator") then makefolder("PrivateServerGenerator") end
...
local RblxReplicatedStorage = game:GetService("RobloxReplicatedStorage");
...
Notification:Notify(
    { Title = 'Private Server Generator', Description = 'Generating a vip for you...' },
    { OutlineColor = Color3.fromRGB(80, 80, 80), Time = 5, Type = 'default' }
)
if getgenv().SendInvites then
    local File = "PrivateServerGenerator/InviteCode_"..os.date("%H%M%S")..".txt";
    writefile(File, tostring(game.PlaceId) .. "\n" .. ac);
end

RblxReplicatedStorage.ContactListIrisInviteTeleport:FireServer(game.PlaceId, "", ac);

local TeleportCheck = false
LocalPlayer.OnTeleport:Connect(function(State)
    if getgenv().SendInvites and (not TeleportCheck) and Queue then
        TeleportCheck = true
        Queue([[
            if not game:IsLoaded() then game.Loaded:Wait() end
            local NotificationHolder = loadstring(
            ...
            Notification:Notify(
                { Title = 'Private Server Generator', Description = 'Would you like to copy invite?' },
                { OutlineColor = Color3.fromRGB(80, 80, 80), Time = 9e9, Type = 'option' },
                {
                    Image = 'http://www.roblox.com/asset/?id=6023426923',
                    ImageColor = Color3.fromRGB(255, 84, 84),
                    Callback = function(Value)
                        if Value == true then
                            local GetFiles = listfiles("PrivateServerGenerator");

                            table.sort(GetFiles);
                            local LatestFile = GetFiles[#GetFiles];

                            local lines = string.split(readfile(LatestFile), "\n");
                            local PlaceID, TokenID = lines[1], lines[2];

                            setclipboard(string.format('game:GetService("RobloxReplicatedStorage").ContactListIrisInviteTeleport:FireServer("%s", "", "%s")',PlaceID,TokenID));
                        end
                    end,
                }
            )
