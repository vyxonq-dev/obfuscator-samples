...
local selectedBodyPart = "Head"  -- Default body part to target
local selectedDevice = "PC"  -- Default device
...
if selectedDevice == "PC" and not Value then
...
local targetPart = player.Character:FindFirstChild(selectedBodyPart)
if targetPart then
local playerTeam = player:FindFirstChild("Status") and player.Status:FindFirstChild("Team")
local localPlayerTeam = localPlayer:FindFirstChild("Status") and localPlayer.Status:FindFirstChild("Team")

if localPlayerTeam and playerTeam and localPlayerTeam.Value ~= playerTeam.Value then
...
-- Handle input for enabling/disabling aimbot if on PC
...
if input.UserInputType == Enum.UserInputType.MouseButton2 then -- Right Mouse Button
print("MouseButton2 Released")
getgenv().AimbotActive = false
end
end)
end

-- RenderStepped loop to handle aimbot aiming
runService.RenderStepped:Connect(function()
if getgenv().AimbotEnabled and (selectedDevice == "Mobile" or getgenv().AimbotActive) then
print("Aimbot Aiming")
local closestPlayer = GetClosestPlayer()
if closestPlayer then
-- Smooth aiming (optional)
local targetPart = closestPlayer.Character:FindFirstChild(selectedBodyPart)
if targetPart then
local targetPosition = targetPart.Position
camera.CFrame = CFrame.new(camera.CFrame.Position, targetPosition)
print("Aiming at:", closestPlayer.Name)
else
print("Target part not found:", selectedBodyPart)
end
else
...
local Players = game:GetService("Players")
local validPlaceIds = {286090429} -- Replace with the correct place IDs
