...
local s, wm = pcall(require, RS.Common.Managers.WeaponManager);
if not s then
    return warn(executor .. " returned an error while trying to require RS.Common.Managers.WeaponManager:\n" .. wm);
end;
...
local getTarget = function(origin: Vector3)
    ...
    for _, char: Model in next, workspace:GetChildren() do
        if char.Name == plr.Name or char:FindFirstChild("ForceField") or not char:FindFirstChild("Humanoid") or char.Humanoid.Health <= 0 or not char:FindFirstChild("Hitbox") then continue; end;

        local tPart: BasePart = char.Hitbox:FindFirstChild("Hitbox_Head") or char.Hitbox:FindFirstChild("Hitbox_Torso") or char.PrimaryPart;
...
local old; old = clonefunction(hookfunction(rawget(wm, "cast"), newcclosure(function(origin, _, ...)
    local c = getTarget(origin);
    if c then
        return old(origin, (c.Position - origin).Unit, ...);
    end;
    return old(origin, _, ...);
end)));
