...
local s, raycast = pcall(require, RS.Modules.Universal.Raycast);
if not s then
    return warn(executor .. " returned an error while trying to require RS.Modules.Universal.Raycast:\n" .. raycast);
end;
...
local old; old = clonefunction(hookfunction(raycast, newcclosure(function(_, origin, __, ___, ____, ...)
    local c = getTarget(origin);
    if c then
        local dir = c.Position - origin;
        return old(_, origin, true, dir.Unit, dir.Magnitude, ...);
    end;
    return old(_, origin, true, ___, ____, ...);
end)));
