local Players = game:GetService("Players")
local player = Players.LocalPlayer
local TweenService = game:GetService("TweenService")

-- SETTINGS
local PREFIX = "Badshah-"
local LOOTLABS_LINK = "https://lootdest.org/s?fcmmeJZf"
local LINKVERTISE_LINK = "https://link-center.net/6147628/ua2rfLp2u1ep"

-- NUMBER CONVERSION
local convert = {
    ["0"] = "X",
    ["1"] = "Z",
    ["2"] = "A",
    ["3"] = "G",
    ["4"] = "W",
    ["5"] = "Q",
    ["6"] = "M",
    ["7"] = "B",
    ["8"] = "O",
    ["9"] = "V"
}

-- GENERATE TODAY KEY
local function generateKey()
    local date = os.date("%d%m%Y")
    local result = ""

    for i = 1, #date do
        local char = date:sub(i, i)
        result = result .. (convert[char] or char)
    end

    return PREFIX .. result
end

local TODAY_KEY = generateKey()

-- NOTIFICATION
local function notify(text)
    pcall(function()
        game.StarterGui:SetCore("SendNotification", {
            Title = "Key System",
            Text = text,
            Duration = 3
        })
    end)
end

-- DISCORD ICON DECODER & FALLBACK
local discordIconAsset = "rbxassetid://15058474299"
pcall(function()
    if writefile and getcustomasset then
        local base64Data = ""
        local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
        local function base64decode(data)
            data = string.gsub(data, '[^'..b..'=]', '')
            return (string.gsub(data, '.', function(x)
                if (x == '=') then return '' end
                local r,f='',(b:find(x)-1)
                for i=6,1,-1 do r=r..(f%2^i-f%2^(i-1)>0 and '1' or '0') end
                return r;
            end):gsub('%d%d%d%d%d%d%d%d', function(x)
                local r=0
                for i=1,8 do r=r+(x:sub(i,i)=='1' and 2^(8-i) or 0) end
                return string.char(r)
            end))
        end
        writefile("discord_icon.png", base64decode(base64Data))
        discordIconAsset = getcustomasset("discord_icon.png")
    end
end)

-- GUI
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "KeySystem"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = player.PlayerGui

-- MAIN FRAME (Adjusted size: width decreased to 230, height to 155)
local Main = Instance.new("Frame")
Main.Name = "MainFrame"
Main.Parent = ScreenGui
Main.Size = UDim2.new(0, 230, 0, 155)
Main.Position = UDim2.new(0.5, -115, 0.5, -77)
Main.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
Main.BorderSizePixel = 0

local MainCorner = Instance.new("UICorner")
MainCorner.CornerRadius = UDim.new(0, 10)
MainCorner.Parent = Main

local MainStroke = Instance.new("UIStroke")
MainStroke.Thickness = 1.5
MainStroke.Color = Color3.fromRGB(35, 35, 35)
MainStroke.Parent = Main

-- TITLE (Decreased width from -60 to -80 to fit the Discord button)
local Title = Instance.new("TextLabel")
Title.Name = "TitleLabel"
Title.Parent = Main
Title.BackgroundTransparency = 1
Title.Size = UDim2.new(1, -80, 0, 25)
Title.Position = UDim2.new(0, 15, 0, 12)
Title.Text = "GET KEY"
Title.TextColor3 = Color3.fromRGB(255, 255, 255)
Title.Font = Enum.Font.GothamBold
Title.TextSize = 13
Title.TextXAlignment = Enum.TextXAlignment.Left

-- CLOSE BUTTON (Changed to 'X' text and styled)
local CloseMain = Instance.new("TextButton")
CloseMain.Name = "CloseButton"
CloseMain.Parent = Main
CloseMain.BackgroundTransparency = 1
CloseMain.Size = UDim2.new(0, 20, 0, 20)
CloseMain.Position = UDim2.new(1, -30, 0, 12)
CloseMain.Text = "X"
CloseMain.Font = Enum.Font.GothamBold
CloseMain.TextSize = 13
CloseMain.TextColor3 = Color3.fromRGB(150, 150, 150)

CloseMain.MouseEnter:Connect(function()
    TweenService:Create(CloseMain, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(255, 75, 75)}):Play()
end)
CloseMain.MouseLeave:Connect(function()
    TweenService:Create(CloseMain, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(150, 150, 150)}):Play()
end)
CloseMain.MouseButton1Click:Connect(function()
    ScreenGui:Destroy()
end)

-- DISCORD BUTTON (Positioned next to the Close button, styled with hover effects)
local DiscordBtn = Instance.new("ImageButton")
DiscordBtn.Name = "DiscordButton"
DiscordBtn.Parent = Main
DiscordBtn.BackgroundTransparency = 1
DiscordBtn.Size = UDim2.new(0, 16, 0, 16)
DiscordBtn.Position = UDim2.new(1, -52, 0, 14)
DiscordBtn.Image = discordIconAsset
DiscordBtn.ImageColor3 = Color3.fromRGB(150, 150, 150)

DiscordBtn.MouseEnter:Connect(function()
    TweenService:Create(DiscordBtn, TweenInfo.new(0.2), {ImageColor3 = Color3.fromRGB(88, 101, 242)}):Play()
end)
DiscordBtn.MouseLeave:Connect(function()
    TweenService:Create(DiscordBtn, TweenInfo.new(0.2), {ImageColor3 = Color3.fromRGB(150, 150, 150)}):Play()
end)
DiscordBtn.MouseButton1Click:Connect(function()
    setclipboard("https://discord.gg/aeMymxbST")
    notify("Discord link copied successfully!")
end)

-- KEY BOX
local KeyBox = Instance.new("TextBox")
KeyBox.Name = "KeyBox"
KeyBox.Parent = Main
KeyBox.Size = UDim2.new(0.86, 0, 0, 32)
KeyBox.Position = UDim2.new(0.07, 0, 0, 45)
KeyBox.PlaceholderText = "Enter Key"
KeyBox.Text = ""
KeyBox.Font = Enum.Font.Gotham
KeyBox.TextSize = 12
KeyBox.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
KeyBox.TextColor3 = Color3.fromRGB(255, 255, 255)
KeyBox.PlaceholderColor3 = Color3.fromRGB(100, 100, 100)
KeyBox.ClearTextOnFocus = true
KeyBox.BorderSizePixel = 0

local KeyCorner = Instance.new("UICorner")
KeyCorner.CornerRadius = UDim.new(0, 6)
KeyCorner.Parent = KeyBox

local KeyStroke = Instance.new("UIStroke")
KeyStroke.Thickness = 1
KeyStroke.Color = Color3.fromRGB(35, 35, 35)
KeyStroke.Parent = KeyBox

KeyBox.Focused:Connect(function()
    TweenService:Create(KeyStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(80, 80, 80)}):Play()
end)
KeyBox.FocusLost:Connect(function()
    TweenService:Create(KeyStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(35, 35, 35)}):Play()
end)

-- BUTTONS CONTAINER
local ButtonsContainer = Instance.new("Frame")
ButtonsContainer.Name = "ButtonsContainer"
ButtonsContainer.Parent = Main
ButtonsContainer.BackgroundTransparency = 1
ButtonsContainer.Size = UDim2.new(0.86, 0, 0, 30)
ButtonsContainer.Position = UDim2.new(0.07, 0, 0, 85)

-- GET KEY BUTTON
local GetKey = Instance.new("TextButton")
GetKey.Name = "GetKeyButton"
GetKey.Parent = ButtonsContainer
GetKey.Size = UDim2.new(0.48, 0, 1, 0)
GetKey.Position = UDim2.new(0, 0, 0, 0)
GetKey.Text = "Get Key"
GetKey.Font = Enum.Font.GothamBold
GetKey.TextSize = 11
GetKey.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
GetKey.TextColor3 = Color3.fromRGB(255, 255, 255)
GetKey.BorderSizePixel = 0

local GetCorner = Instance.new("UICorner")
GetCorner.CornerRadius = UDim.new(0, 6)
GetCorner.Parent = GetKey

local GetStroke = Instance.new("UIStroke")
GetStroke.Thickness = 1
GetStroke.Color = Color3.fromRGB(45, 45, 45)
GetStroke.Parent = GetKey

GetKey.MouseEnter:Connect(function()
    TweenService:Create(GetKey, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}):Play()
    TweenService:Create(GetStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(70, 70, 70)}):Play()
end)
GetKey.MouseLeave:Connect(function()
    TweenService:Create(GetKey, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(25, 25, 25)}):Play()
    TweenService:Create(GetStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(45, 45, 45)}):Play()
end)

-- SUBMIT BUTTON
local Submit = Instance.new("TextButton")
Submit.Name = "SubmitButton"
Submit.Parent = ButtonsContainer
Submit.Size = UDim2.new(0.48, 0, 1, 0)
Submit.Position = UDim2.new(0.52, 0, 0, 0)
Submit.Text = "Submit Key"
Submit.Font = Enum.Font.GothamBold
Submit.TextSize = 11
Submit.BackgroundColor3 = Color3.fromRGB(219, 75, 75)
Submit.TextColor3 = Color3.fromRGB(255, 255, 255)
Submit.BorderSizePixel = 0

local SubmitCorner = Instance.new("UICorner")
SubmitCorner.CornerRadius = UDim.new(0, 6)
SubmitCorner.Parent = Submit

Submit.MouseEnter:Connect(function()
    TweenService:Create(Submit, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(245, 85, 85)}):Play()
end)
Submit.MouseLeave:Connect(function()
    TweenService:Create(Submit, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(219, 75, 75)}):Play()
end)

-- FOOTER (Changed to "Tora IsMe" as requested)
local Footer = Instance.new("TextLabel")
Footer.Name = "FooterLabel"
Footer.Parent = Main
Footer.BackgroundTransparency = 1
Footer.Size = UDim2.new(1, -30, 0, 15)
Footer.Position = UDim2.new(0, 15, 1, -22)
Footer.Text = "Tora IsMe"
Footer.TextColor3 = Color3.fromRGB(150, 150, 150)
Footer.Font = Enum.Font.GothamBold
Footer.TextSize = 10
Footer.TextXAlignment = Enum.TextXAlignment.Left

-- GET KEY OPTIONS POPUP (DRAWER PANEL)
local OptionsFrame = Instance.new("Frame")
OptionsFrame.Name = "OptionsFrame"
OptionsFrame.Parent = Main
OptionsFrame.Size = UDim2.new(0.86, 0, 0, 0) -- Starts closed
OptionsFrame.Position = UDim2.new(0.07, 0, 0, 45)
OptionsFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
OptionsFrame.BorderSizePixel = 0
OptionsFrame.ClipsDescendants = true
OptionsFrame.Visible = false

local OptionsCorner = Instance.new("UICorner")
OptionsCorner.CornerRadius = UDim.new(0, 6)
OptionsCorner.Parent = OptionsFrame

local OptionsStroke = Instance.new("UIStroke")
OptionsStroke.Thickness = 1
OptionsStroke.Color = Color3.fromRGB(45, 45, 45)
OptionsStroke.Parent = OptionsFrame

local PopTitle = Instance.new("TextLabel")
PopTitle.Name = "PopTitle"
PopTitle.Parent = OptionsFrame
PopTitle.BackgroundTransparency = 1
PopTitle.Size = UDim2.new(1, -30, 0, 25)
PopTitle.Position = UDim2.new(0, 10, 0, 5)
PopTitle.Text = "Select Key Provider"
PopTitle.TextColor3 = Color3.fromRGB(200, 200, 200)
PopTitle.Font = Enum.Font.GothamBold
PopTitle.TextSize = 10
PopTitle.TextXAlignment = Enum.TextXAlignment.Left

local ClosePopup = Instance.new("TextButton")
ClosePopup.Name = "ClosePopup"
ClosePopup.Parent = OptionsFrame
ClosePopup.BackgroundTransparency = 1
ClosePopup.Size = UDim2.new(0, 20, 0, 20)
ClosePopup.Position = UDim2.new(1, -25, 0, 7)
ClosePopup.Text = "X"
ClosePopup.Font = Enum.Font.GothamBold
ClosePopup.TextSize = 11
ClosePopup.TextColor3 = Color3.fromRGB(150, 150, 150)

ClosePopup.MouseEnter:Connect(function()
    TweenService:Create(ClosePopup, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(255, 75, 75)}):Play()
end)
ClosePopup.MouseLeave:Connect(function()
    TweenService:Create(ClosePopup, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(150, 150, 150)}):Play()
end)

local LinkContainer = Instance.new("Frame")
LinkContainer.Name = "LinkContainer"
LinkContainer.Parent = OptionsFrame
LinkContainer.BackgroundTransparency = 1
LinkContainer.Size = UDim2.new(1, -20, 0, 28)
LinkContainer.Position = UDim2.new(0, 10, 0, 32)

local Lootlabs = Instance.new("TextButton")
Lootlabs.Name = "LootlabsButton"
Lootlabs.Parent = LinkContainer
Lootlabs.Size = UDim2.new(0.48, 0, 1, 0)
Lootlabs.Position = UDim2.new(0, 0, 0, 0)
Lootlabs.Text = "LootLabs"
Lootlabs.Font = Enum.Font.GothamBold
Lootlabs.TextSize = 10
Lootlabs.BackgroundColor3 = Color3.fromRGB(100, 50, 180)
Lootlabs.TextColor3 = Color3.fromRGB(255, 255, 255)
Lootlabs.BorderSizePixel = 0

local LootCorner = Instance.new("UICorner")
LootCorner.CornerRadius = UDim.new(0, 6)
LootCorner.Parent = Lootlabs

Lootlabs.MouseEnter:Connect(function()
    TweenService:Create(Lootlabs, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(120, 60, 210)}):Play()
end)
Lootlabs.MouseLeave:Connect(function()
    TweenService:Create(Lootlabs, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(100, 50, 180)}):Play()
end)

local Linkvertise = Instance.new("TextButton")
Linkvertise.Name = "LinkvertiseButton"
Linkvertise.Parent = LinkContainer
Linkvertise.Size = UDim2.new(0.48, 0, 1, 0)
Linkvertise.Position = UDim2.new(0.52, 0, 0, 0)
Linkvertise.Text = "Linkvertise"
Linkvertise.Font = Enum.Font.GothamBold
Linkvertise.TextSize = 10
Linkvertise.BackgroundColor3 = Color3.fromRGB(200, 90, 30)
Linkvertise.TextColor3 = Color3.fromRGB(255, 255, 255)
Linkvertise.BorderSizePixel = 0

local LinkCorner = Instance.new("UICorner")
LinkCorner.CornerRadius = UDim.new(0, 6)
LinkCorner.Parent = Linkvertise

Linkvertise.MouseEnter:Connect(function()
    TweenService:Create(Linkvertise, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(220, 110, 40)}):Play()
end)
Linkvertise.MouseLeave:Connect(function()
    TweenService:Create(Linkvertise, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(200, 90, 30)}):Play()
end)

-- GET KEY POPUP TOGGLE
local isPopupOpen = false
GetKey.MouseButton1Click:Connect(function()
    if isPopupOpen then return end
    isPopupOpen = true
    OptionsFrame.Visible = true
    
    -- Animate open (accordion style)
    TweenService:Create(OptionsFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0.86, 0, 0, 72)
    }):Play()
end)

local function closePopupFunc()
    if not isPopupOpen then return end
    
    local tween = TweenService:Create(OptionsFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        Size = UDim2.new(0.86, 0, 0, 0)
    })
    tween:Play()
    tween.Completed:Connect(function()
        OptionsFrame.Visible = false
        isPopupOpen = false
    end)
end

ClosePopup.MouseButton1Click:Connect(closePopupFunc)

-- LINK ACTIONS
Lootlabs.MouseButton1Click:Connect(function()
    setclipboard(LOOTLABS_LINK)
    notify("LootLabs link copied to clipboard")
    closePopupFunc()
end)

Linkvertise.MouseButton1Click:Connect(function()
    setclipboard(LINKVERTISE_LINK)
    notify("Linkvertise link copied to clipboard")
    closePopupFunc()
end)

-- SUBMIT KEY
Submit.MouseButton1Click:Connect(function()
    if KeyBox.Text == TODAY_KEY then
        notify("Key Verified Successfully")
        task.wait(1)
        ScreenGui:Destroy()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/gumanba/Scripts/main/ThrowaCoin"))()
    else
        notify("Invalid Key")
        KeyBox.Text = ""
    end
end)

-- NOTIFICATION ON RUN (Join Discord notification)
notify("Join our Discord for more Roblox scripts")
