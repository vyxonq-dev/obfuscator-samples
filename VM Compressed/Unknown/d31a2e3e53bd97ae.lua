...
game.ReplicatedStorage["01_server"]:FireServer("cmd", "-gh 48474313, 62724852, 451220849, 376527115, 3409612660, 3940375351, 3033910400, 4047554959, 63690008, 62234425")
...
if lps == 168556275 then game:GetService("Players").LocalPlayer:Kick("wrong game search: [UP] Just a baseplate. - creator: the local maze") end
...
_G.MobileInput = _G.MobileInput or {
	W = false,
	A = false,
	S = false,
	D = false,
...
if UserInputService.TouchEnabled then
	...
	local function bind(btn, key)
		btn.MouseButton1Down:Connect(function()
			_G.MobileInput[key] = true
		end)
		btn.MouseButton1Up:Connect(function()
			_G.MobileInput[key] = false
		end)
	end

	local W = makeButton("W", UDim2.new(0.35,0,0,0))
	local A = makeButton("A", UDim2.new(0,0,0.35,0))
	local S = makeButton("S", UDim2.new(0.35,0,0.35,0))
	local D = makeButton("D", UDim2.new(0.7,0,0.35,0))
	local Q = makeButton("Q", UDim2.new(0,0,0.7,0))
	local E = makeButton("E", UDim2.new(0.7,0,0.7,0))
...
local function isDown(key, enum)
	return UserInputService:IsKeyDown(enum) or (_G.MobileInput and _G.MobileInput[key])
end
...
while wait(0.01) do
	...
	if isDown("A", Enum.KeyCode.A) and moveB == 0 and moveF == 0 and moveR == 0 and moveU == 0 and moveD == 0 then
		moveL = 15
		check = 1
		speed = speed + 0.05
	end

	if isDown("D", Enum.KeyCode.D) and moveB == 0 and moveL == 0 and moveF == 0 and moveU == 0 and moveD == 0 then
		moveR = 15
		check = 1
		speed = speed + 0.05
	end

	if isDown("W", Enum.KeyCode.W) and moveB == 0 and moveL == 0 and moveR == 0 and moveU == 0 and moveD == 0 then
		speed = speed + 0.05
		check = 1
		moveF = 15
	end

	if isDown("S", Enum.KeyCode.S) and moveF == 0 and moveL == 0 and moveR == 0 and moveU == 0 and moveD == 0 then
		moveB = 15
		check = 1
		speed = speed + 0.05
	end

	if isDown("Q", Enum.KeyCode.Q) and moveB == 0 and moveL == 0 and moveR == 0 and moveF == 0 and moveU == 0 then
		moveD = 15
		check = 1
		speed = speed + 0.05
	end

	if isDown("E", Enum.KeyCode.E) and moveB == 0 and moveL == 0 and moveR == 0 and moveD == 0 and moveF == 0 then
		moveU = 15
		check = 1
		speed = speed + 0.05
	end
