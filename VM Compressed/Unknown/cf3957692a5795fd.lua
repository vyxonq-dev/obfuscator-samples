...
function Library:SetUIBind(enum)
    pcall(function()
        if enum == nil then
            UIToggle = Enum.KeyCode.F
            toggleBind = Enum.KeyCode.F
            UIToggle2()
        else
            UIToggle = enum
            toggleBind = enum
...
--Check if keyBind is loaded
spawn(function()
	while true do
		wait(1)
		pcall(function()
			if game.CoreGui.dark_UI.main.keyBind then
				keyBindButton = game.CoreGui.dark_UI.main.keyBind
			end
		end)
	end
end)
...
spawn(function()
	while true do
		wait()
		if keyBindButton ~= nil then
			keyBindButton.MouseButton1Down:Connect(function()
				keyBindButton.Text = "| ... |"
				if UIToggleEvent then
					UIToggleEvent:Disconnect()
				end
...
function Library:Create(table)
    ...
    title.Position = UDim2.fromScale(0.2, 0.054)
    ...
    keyBind.Text = "| " .. tostring(UIToggle):split(".")[3] .. " |"
    keyBind.TextColor3 = Color3.fromRGB(168, 168, 168)
    keyBind.TextSize = 18
    keyBind.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    keyBind.BackgroundTransparency = 1
    keyBind.Position = UDim2.fromScale(0.739, 0.05)
    keyBind.Size = UDim2.fromOffset(83, 28)
    keyBind.Parent = main
    ...
    slider.MouseButton1Down:Connect(function()
        ...
        while RunService.RenderStepped:Wait() and down do
            ...
            local tween = TweenService:Create(frame1, TweenInfo.new(0.06), {Size = UDim2.fromScale(percentage, 1)}):Play()
            callback(value)
        end
    end)
end
